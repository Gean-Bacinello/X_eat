...
<div class="container">
  ...
  <div class="d-flex justify-content-between mb-4">
    ...
    <form action="{{ route('alimentos.index') }}" method="GET" class="d-flex align-items-center">
      <input type="text" name="search" class="form-control me-2" placeholder="Pesquisar Alimento" value="{{ request('search') }}">
      ...
      <div class="d-flex align-items-center justify-content-between p-3 border rounded shadow-sm bg-white">
        ...
        <div class="me-4" style="width: 250px; overflow: hidden;">
          <h5 class="mb-1 fw-semibold text-truncate" title="{{ $alimento->Nome_Alimento }}">
            <a href="{{ route('alimentos.show', $alimento->ID_Alimento)}}"> {{ $alimento->Nome_Alimento }}</a>
        ...
        <div class="d-flex gap-2">
          <a href="{{ route('alimentos.edit', $alimento->ID_Alimento) }}" class="btn btn-sm btn-outline-primary">Editar</a>
          <form action="{{ route('alimentos.destroy', $alimento->ID_Alimento) }}" method="POST">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Tem certteza que deseeja apagar')">Excluir</button>
          </form>
        </div>
